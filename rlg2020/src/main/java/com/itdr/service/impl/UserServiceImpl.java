...
public class UserServiceImpl implements UserService {
    ...
    public ServerResponse<User> register(User u) {
        ...
        //查找用户是否存在
        ServerResponse<User> username = check(u.getUsername(), "username");
        //查找邮箱是否存在
        ServerResponse<User> email = check(u.getEmail(), "email");
    ...
    public ServerResponse<User> check(String str,String type) {
        ...
        if (StringUtils.isEmpty(str)){
            return ServerResponse.defeatedRS(
                    ...
                    ConstCode.UserEnum.EMPTY_STR.getDesc());
        }
    ...
    @Override
    public ServerResponse<User> updateInfo(Integer phonenumber, String email, String raddress,User user) {

        User u = new User();
        u.setId(user.getId());
        u.setPhonenumber(phonenumber);
        u.setEmail(email);
        u.setRaddress(raddress);
        int i = userMapper.updateByPrimaryKeySelective(u);
        if (i < 0){
            return ServerResponse.defeatedRS(
                    ConstCode.UserEnum.LOSE_UPDATE.getCode(),
                    ConstCode.UserEnum.LOSE_UPDATE.getDesc());
        }
    ...
    @Override
    public ServerResponse<User> restPassword(User user,String passwordOld, String passwordNew) {
        //参数非空判断
