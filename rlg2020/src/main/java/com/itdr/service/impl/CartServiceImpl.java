...
public class CartServiceImpl implements CartService {
    ...
    @Override
    public ServerResponse getOne(User user, Integer cartId) {
        Cart cart = cartMapper.selectByUserIdAndCartId(user.getId(),cartId);
        if (cart == null){
            return ServerResponse.defeatedRS(
                    ConstCode.ProductEnum.INEXISTENCE_PRODUCT.getCode(),
                    ConstCode.ProductEnum.INEXISTENCE_PRODUCT.getDesc());
        }
        Product p = productMapper.selectByProductId(cart.getProductId());
        CartProductVO cpv = ObjectToVOUtil.cartAndProductToVO(cart,p);
        return ServerResponse.successRS(cpv);
    }
}
